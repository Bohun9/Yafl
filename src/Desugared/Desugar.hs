...
groupCtors (eq@(CtorEquation c ps1 _ _) : eqs) = do
  ...
  reducedEqs <- mapM (\(CtorEquation _ ps1' ps2' e') ->
                       if length ps1 == length ps1' then
                         return $ Equation (ps1' ++ ps2') e'
                       else
                         throwError $ "A different number of subpatterns were specified for the constructor " ++ c
                       ) g'
...
desugarExpr' e p =
  case e of
    ...
    S.EFun f x e1 e2   -> D.EFun <$> return f <*> return x <*> desugarExpr e1 <*> desugarExpr e2
